...
SingleReplicaCompaction::SingleReplicaCompaction(const TabletSharedPtr& tablet)
        : _tablet(tablet),
          ...
          _input_index_size(0),
          _state(SingleReplicaCompactionState::INITED) {
#ifndef BE_TEST
...
Status SingleReplicaCompaction::prepare_compact() {
    ...
    std::unique_lock<std::mutex> lock_cumu(_tablet->get_cumulative_compaction_lock(), std::try_to_lock);
    if (!lock_cumu.owns_lock()) {
        LOG(INFO) << "The tablet is under cumulative compaction. tablet=" << _tablet->full_name();
        return Status::Error<TRY_LOCK_FAILED>();
    }
    LOG(INFO)  << _tablet->tablet_id() <<  " got cumulative compaction lock";
    ...
    LOG(INFO) << _tablet->tablet_id() <<  " got base compaction lock";

    RETURN_NOT_OK(_pick_rowsets_to_compact());
    LOG(INFO) <<  _tablet->tablet_id() << "rowsets picked, size is: " << _input_rowsets.size();
...
Status SingleReplicaCompaction::_pick_rowsets_to_compact() {
...
Status SingleReplicaCompaction::execute_compact() {
    ...
    VLOG_CRITICAL << _tablet->tablet_id() << " execute single replica compaction";

    Status st = _execute_compact_impl();
    if (!st.ok()) {
        _gc_output_rowset();
    }
    return st;
...
void SingleReplicaCompaction::_gc_output_rowset() {
    if (_state != SingleReplicaCompactionState::SUCCESS && _output_rowset != nullptr) {
...
Status SingleReplicaCompaction::_execute_compact_impl() {
    std::unique_lock<std::mutex> lock_cumu(_tablet->get_cumulative_compaction_lock(), std::try_to_lock);
    if (!lock_cumu.owns_lock()) {
        LOG(INFO) << "The tablet is under cumulative compaction. tablet=" << _tablet->full_name();
        return Status::Error<TRY_LOCK_FAILED>();
    ...
    if (_tablet->get_clone_occurred()) {
        _tablet->set_clone_occurred(false);
        LOG(INFO) << _tablet->tablet_id() <<  " tablet is cloned single replica failed";
        return Status::Error<BE_CLONE_OCCURRED>();
    }

    SCOPED_ATTACH_TASK(_mem_tracker);

    RETURN_NOT_OK(_do_compaction());
    LOG(INFO) << _tablet->tablet_id() <<  " single replica compaction finished";

    _state = SingleReplicaCompactionState::SUCCESS;

    return Status::OK();
}

Status SingleReplicaCompaction::_do_compaction() {
    TBackend addr;
    std::string token;
    if(!StorageEngine::instance()->get_tbackend(_tablet->tablet_id(), addr, token)) {
        LOG(INFO) << _tablet->tablet_id() <<  " tablet don't have master peer";
    ...
    if(_should_fetch_from_peer(peer_versions)) {
        _tablet->data_dir()->disks_compaction_num_increment(1);
        Status st = _do_compaction_impl(addr, token);
        _tablet->data_dir()->disks_compaction_num_increment(-1);
        return st;
    }
    
    VLOG_CRITICAL << "single replica compaction don't need to fetch. tablet=" << _tablet->full_name();
    return Status::OK();
}

//  master_versions: 0-10 11-11 12-14 15-18
//  local_versions:  0-10 11-11 12-12 13-13 14-14 15-18
bool SingleReplicaCompaction::_should_fetch_from_peer(std::vector<Version>& peer_versions){
    ...
    if (peer_versions[index_peer].contains(local_versions[index_local])) {
        ...
        if (local_versions[index_local - 1].second == peer_versions[index_peer].second &&
            peer_versions[index_peer].first != peer_versions[index_peer].second) {
            VLOG_CRITICAL << _tablet->tablet_id() << " tablet should fetch peer replica, version from " 
                        << peer_versions[index_peer].first << " to " << peer_versions[index_peer].second;
            find = true;
            _output_version = peer_versions[index_peer];
...
Status SingleReplicaCompaction::_do_compaction_impl(TBackend& addr, std::string& token) {
    RETURN_IF_ERROR(_fetch_compaction_result(addr, token));
    _adjust_input_rowset();
    RETURN_NOT_OK(_modify_rowsets());
...
Status SingleReplicaCompaction::_fetch_compaction_result(TBackend& addr, std::string& token) {
    LOG(INFO) << "begin to fetch compaction result, tablet_id=" << _tablet->tablet_id()
              << ", addr=" << addr.host << ", version=" << _output_version;
    ...
    auto st = _make_snapshot(addr.host, addr.be_port, _tablet->tablet_id(), _tablet->schema_hash(),
                             timeout_s, _output_version, &snapshot_path);
    if (st.ok()) {
        status = Status::OK();
    } else {
    ...
    if (status.ok()) {
        Status olap_status = _finish_clone(local_data_path, _output_version);
        if (!olap_status.ok()) {
...
Status SingleReplicaCompaction::_make_snapshot(const std::string& ip, int port, TTableId tablet_id,
                                  ...
                                  std::string* snapshot_path) {
    LOG(WARNING)  << "single replica compaction make snapshot ip=" << ip << ", tablet_id=" << tablet_id;
...
Status SingleReplicaCompaction::_download_files(DataDir* data_dir, const std::string& remote_url_prefix,
    ...
    VLOG_DEBUG << "single replica compaction begin to download files, remote path=" << remote_url_prefix
...
Status SingleReplicaCompaction::_finish_clone(const string& clone_dir, const Version& output_version) {
    ...
    std::vector<string> linked_success_files;
    {
        do {
            ...
            auto cloned_tablet_meta_file = fmt::format("{}/{}.hdr", clone_dir, _tablet->tablet_id());
            ...
            for (const string& clone_file : clone_file_names) {
                if (local_file_names.find(clone_file) != local_file_names.end()) {
                    VLOG_NOTICE << "find same file when clone, skip it. "
                                << "tablet=" << _tablet->full_name() << ", clone_file=" << clone_file;
                    continue;
                }
...
void SingleReplicaCompaction::_adjust_input_rowset() {
    VLOG_DEBUG << "adjust input rowset, version=" << _output_version;
    auto rs_iter = _input_rowsets.begin();
    while (rs_iter != _input_rowsets.end()) {
        if (_output_version.contains((*rs_iter)->version())) {
            ++rs_iter;
            continue;
        }
...
Status SingleReplicaCompaction::_modify_rowsets() {
